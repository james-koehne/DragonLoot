...
- [x] Don't move too many coins to digging location
...
- Dig/deposit brush feel is player-owned via `PlayerControllerDefinition` (`carveRadius`, `carveVolumeScale`, min-radius fraction, re-blur pad/passes/strength, falloff sharpness). Pile defs no longer carry carve radius/volume.
...
- `Assets/Code/Graphics/GoldPile/GoldPileCarveSettings.cs`
- `Assets/Code/Graphics/GoldPile/GoldPileHeightfield.cs`
...
- `Assets/Code/Interaction/TreasurePileVisual.cs`
- `Assets/Code/Interaction/TreasurePileInteractable.cs`
- `Assets/Code/Debug/Sections/DebugGoldPileSection.cs`
- `Assets/Code/Graphics/Editor/GoldPileMaterialInstaller.cs`
...
8. On `PlayerControllerDefinition`, raise `carveBlurPasses` / `carveBlurStrength` and dig — dent skirts should soften more; set passes to 0 — digs should look sharper; raise `carveFalloffSharpness` — center should dig tighter.
...
- Tune dig brush on `PlayerControllerDefinition` (not pile defs): radius/volume, `carveBlurPadCells`, `carveBlurPasses`, `carveBlurStrength`, `carveFalloffSharpness`.
