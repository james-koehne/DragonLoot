#ifndef DRAGONLOOT_ENVIRONMENT_LIT_GRID_LIGHTING_INCLUDED
#define DRAGONLOOT_ENVIRONMENT_LIT_GRID_LIGHTING_INCLUDED
...
void EnvironmentLitGridInitializeBakedGIData(Varyings input, inout InputData inputData)
{
...
void EnvironmentLitGridInitializeInputData(Varyings input, half3 normalWS, out InputData inputData)
{
    ...
    inputData.shadowMask = SAMPLE_SHADOWMASK(input.staticLightmapUV);
    EnvironmentLitGridInitializeBakedGIData(input, inputData);
}

Varyings EnvironmentLitGridVert(Attributes input)
{
...
half4 EnvironmentLitGridFrag(Varyings input) : SV_Target
{
    UNITY_SETUP_INSTANCE_ID(input);
    ...
    half3 albedo = WorldGridBlendAlbedo(albedoSample.rgb, input.positionWS, normalWS);
    ...
    InputData inputData;
    EnvironmentLitGridInitializeInputData(input, normalWS, inputData);
